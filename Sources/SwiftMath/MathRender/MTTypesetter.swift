...
func getInterElementSpaceArrayIndexForType(_ type: MTMathAtomType, row: Bool) -> Int {
    ...
    case .radical:
        if row {
            ...
            assertionFailure("Interelement space undefined for radical on the right. Treat radical as ordinary.")
            return Int.max
        }
    default:
        assertionFailure("Interelement space undefined for type \(type)")
        return Int.max
    }
}
...
class MTTypesetter {
    ...
    static func createLineForMathList(_ mathList: MTMathList?, font: MTFont?, style: MTLineStyle) -> MTMathListDisplay? {
        ...
        return createLineForMathList(finalizedList, font: font, style: style, cramped: false)
    }

    // Internal
    static func createLineForMathList(_ mathList: MTMathList?, font: MTFont?, style: MTLineStyle, cramped: Bool) -> MTMathListDisplay? {
        return createLineForMathList(mathList, font: font, style: style, cramped: cramped, spaced: false)
    }
    ...
    static func createLineForMathList(_ mathList: MTMathList?, font: MTFont?, style: MTLineStyle, cramped: Bool, spaced: Bool) -> MTMathListDisplay? {
        assert(font != nil)
        let preprocessedAtoms = preprocessMathList(mathList)
    ...
    func createDisplayAtoms(_ preprocessed: [MTMathAtom]) {
        ...
        for atom in preprocessed {
            ...
            case .radical:
                ...
                addInterElementSpace(prevNode, currentType: .ordinary)
                let displayRad = makeRadical(rad.radicand, range: rad.indexRange)
                if rad.degree != nil {
                ...
                if atom.subScript != nil || atom.superScript != nil {
                    makeScripts(atom, display: displayRad, index: UInt(rad.indexRange.location), delta: 0)
                }
            ...
            case .fraction:
                ...
                let display = makeFraction(frac)
                displayAtoms.append(display!)
                currentPosition.x += display!.width
                // add super scripts || subscripts
                if atom.subScript != nil || atom.superScript != nil {
                    makeScripts(atom, display: display, index: UInt(frac!.indexRange.location), delta: 0)
                }
            ...
            case .inner:
                ...
                // add super scripts || subscripts
                if atom.subScript != nil || atom.superScript != nil {
                    makeScripts(atom, display: display, index: UInt(atom.indexRange.location), delta: 0)
                }
            ...
            case .underline:
                ...
                let display = makeUnderline(under)
                displayAtoms.append(display!)
                currentPosition.x += display!.width
                // add super scripts || subscripts
                if atom.subScript != nil || atom.superScript != nil {
            ...
            case .table:
                ...
                addInterElementSpace(prevNode, currentType: .inner)
                atom.type = .inner

                let table = atom as! MTMathTable?
                let display = makeTable(table)
                displayAtoms.append(display!)
                currentPosition.x += display!.width
            ...
            case .ordinary, .binaryOperator, .relation, .open, .close, .placeholder, .punctuation:
                ...
                if atom.subScript != nil || atom.superScript != nil {
                    ...
                    if !atom.nucleus.isEmpty {
                        ...
                        let glyph = findGlyphForCharacterAtIndex(index, inString: atom.nucleus)
                    ...
                    makeScripts(atom, display: line, index: UInt(NSMaxRange(atom.indexRange) - 1), delta: delta)
                }
            } // switch
            lastType = atom.type
            prevNode = atom
        ...
        if spaced && lastType != nil {
            ...
            let interElementSpace = getInterElementSpace(lastType, right: .close)
    ...
    func makeScripts(_ atom: MTMathAtom?, display: MTDisplay?, index: UInt, delta: CGFloat) {
        ...
        if atom!.superScript == nil {
            ...
            let _subscript = MTTypesetter.createLineForMathList(atom!.subScript, font: font, style: scriptStyle(), cramped: subscriptCramped())
        ...
        let superScript = MTTypesetter.createLineForMathList(atom!.superScript, font: font, style: scriptStyle(), cramped: superScriptCramped())
        ...
        let ssubscript = MTTypesetter.createLineForMathList(atom!.subScript, font: font, style: scriptStyle(), cramped: subscriptCramped())
    ...
    func addDelimitersToFractionDisplay(_ display: MTFractionDisplay?, forFraction frac: MTFraction?) -> MTDisplay? {
        ...
        if !frac!.leftDelimiter.isEmpty {
            let leftGlyph = findGlyphForBoundary(frac!.leftDelimiter, withHeight: glyphHeight())
        ...
        if !frac!.rightDelimiter.isEmpty {
            let rightGlyph = findGlyphForBoundary(frac!.rightDelimiter, withHeight: glyphHeight())
    ...
    func getRadicalGlyphWithHeight(_ radicalHeight: CGFloat) -> MTDisplayDS? {
        ...
        let radicalGlyph = findGlyphForCharacterAtIndex("\u{221A}".startIndex, inString: "\u{221A}")
        let glyph = findGlyph(radicalGlyph, withHeight: radicalHeight, glyphAscent: &glyphAscent, glyphDescent: &glyphDescent, glyphWidth: &glyphWidth)
    ...
    func constructGlyph(_ glyph: CGGlyph, withHeight glyphHeight: CGFloat) -> MTGlyphConstructionDisplay? {
        ...
        constructGlyphWithParts(parts, glyphHeight: glyphHeight, glyphs: &glyphs, offsets: &offsets, height: &height)
    ...
    func makeLargeOp(_ op: MTLargeOperator!) -> MTDisplay? {
        ...
        if op.nucleus.count == 1 {
            ...
            return addLimitsToDisplay(glyphDisplay, forOperator: op, delta: delta)
        } else {
    ...
    func addLimitsToDisplay(_ display: MTDisplay?, forOperator op: MTLargeOperator, delta: CGFloat) -> MTDisplay? {
        ...
        if op.limits && style == .display {
            // make limits
            var superScript: MTMathListDisplay?, subScript: MTMathListDisplay?
            if op.superScript != nil {
                superScript = MTTypesetter.createLineForMathList(op.superScript, font: font, style: scriptStyle(), cramped: superScriptCramped())
            }
            if op.subScript != nil {
                subScript = MTTypesetter.createLineForMathList(op.subScript, font: font, style: scriptStyle(), cramped: subscriptCramped())
            }
            ...
            currentPosition.x += display!.width
            makeScripts(op, display: display, index: UInt(op.indexRange.location), delta: delta)
            return display
        }
    }
    ...
    func makeLeftRight(_ inner: MTInner?) -> MTDisplay? {
        ...
        if inner!.leftBoundary != nil && !inner!.leftBoundary!.nucleus.isEmpty {
            let leftGlyph = findGlyphForBoundary(inner!.leftBoundary!.nucleus, withHeight: glyphHeight)
            leftGlyph!.position = position
            position.x += leftGlyph!.width
            innerElements.append(leftGlyph!)
        }
        ...
        if inner!.rightBoundary != nil && !inner!.rightBoundary!.nucleus.isEmpty {
            let rightGlyph = findGlyphForBoundary(inner!.rightBoundary!.nucleus, withHeight: glyphHeight)
            rightGlyph!.position = position
            position.x += rightGlyph!.width
            innerElements.append(rightGlyph!)
        }
    ...
    func findGlyphForBoundary(_ delimiter: String, withHeight glyphHeight: CGFloat) -> MTDisplay? {
        ...
        let glyph = findGlyph(leftGlyph, withHeight: glyphHeight, glyphAscent: &glyphAscent, glyphDescent: &glyphDescent, glyphWidth: &glyphWidth)
    ...
    func getSkew(_ accent: MTAccent?, accenteeWidth width: CGFloat, accentGlyph: CGGlyph) -> CGFloat {
        ...
        if !isSingleCharAccentee(accent) {
            ...
            let innerAtom = accent.innerList!.atoms[0]
            let accenteeGlyph = findGlyphForCharacterAtIndex(innerAtom.nucleus.index(innerAtom.nucleus.endIndex, offsetBy: -1), inString: innerAtom.nucleus)
    ...
    func makeAccent(_ accent: MTAccent?) -> MTDisplay? {
        ...
        var accentGlyph = findGlyphForCharacterAtIndex(end, inString: accent!.nucleus)
        let accenteeWidth = accentee!.width
        var glyphAscent = CGFloat(0), glyphDescent = CGFloat(0), glyphWidth = CGFloat(0)
        accentGlyph = findVariantGlyph(accentGlyph, withMaxWidth: accenteeWidth, maxWidth: &glyphAscent, glyphDescent: &glyphDescent, glyphWidth: &glyphWidth)
    ...
    func makeTable(_ table: MTMathTable?) -> MTDisplay? {
        ...
        for row in displays {
            let rowDisplay = makeRowWithColumns(row, forTable: table, columnWidths: columnWidths)
